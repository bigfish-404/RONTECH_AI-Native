        ...
        try {
            ...
            try {
                ...
                if ($request.Method -eq 'POST') {
                    ...
                    if ($request.Path -in @('/api/save', '/api/generate', '/api/select-output-folder', '/api/output-folder-selection', '/api/open-output')) {
                ...
                switch ($route) {
                    ...
                    'GET /api/data' {
                        $data = Read-OrderData
                        Send-JsonResponse -Stream $stream -StatusCode 200 -Data @{ ok = $true; targetMonth = $data.targetMonth; records = $data.records; outputRoot = $outputRoot }
                    }
                    'POST /api/save' {
