...
function Invoke-OrderWorkbookGeneration {
    param(
    ...
    $basePath = [IO.Path]::GetFullPath($AppDirectory)
    $unifiedTemplatePath = [IO.Path]::GetFullPath($TemplatePath)
    $outputRoot = [IO.Path]::GetFullPath($WorkingOutputRoot)
    ...
    if ($null -ne $recordsProperty -and $null -ne $recordsProperty.Value) {
        [object[]]$inputRecords = @($recordsProperty.Value)
    }
