...
function validate(requireRecords = true) {
  ...
  [...new Set(state.records.map((r) => text(r.宛先会社名)).filter(Boolean))].forEach((company) => {
    ...
    if (folderOwners.has(key(folder)) && folderOwners.get(key(folder)) !== company) errors.push(`異なる会社で同じ出力フォルダ名は使用できません: ${folderOwners.get(key(folder))} / ${company}`);
    else folderOwners.set(key(folder), company);
  });
...
async function generateOrders() {
  ...
  if (!window.confirm(`${state.targetMonth} の注文書を作成します。よろしいですか？`)) return;
...
function addRow() {
  ...
  requestAnimationFrame(() => elements.tbody.querySelector(`tr[data-row-id="${CSS.escape(record._id)}"] input`)?.focus());
}
function duplicateSelected() {
  const copies = state.records.filter((r) => state.selectedIds.has(r._id)).map((r) => ({ ...r, _id: makeId() }));
