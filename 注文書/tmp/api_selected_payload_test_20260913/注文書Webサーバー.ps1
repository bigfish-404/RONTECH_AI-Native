...
$pathRoot = [IO.Path]::GetPathRoot($basePath)
if ($basePath.Length -gt $pathRoot.Length) {
    $basePath = $basePath.TrimEnd([IO.Path]::DirectorySeparatorChar, [IO.Path]::AltDirectorySeparatorChar)
}
...
$toolSignature = 'rontech-order-tool-v1'
$script:serverRunning = $true
$script:lastGeneratedOutputPath = ''
...
function Invoke-OrderGeneration {
    ...
    $resolvedOutputRoot = if ([string]::IsNullOrWhiteSpace($requestedOutputRoot)) {
        $outputRoot
    }
    else {
        ...
        $candidateOutputRoot = [IO.Path]::GetFullPath($requestedOutputRoot)
        if (-not (Test-Path -LiteralPath $candidateOutputRoot -PathType Container)) {
    ...
    try {
        $generatedOutput = @(& $scriptBlock -BaseDirectory $basePath -OrderData $Data -OutputDirectory $resolvedOutputRoot 6>&1 5>&1 4>&1 3>&1 2>&1)
    ...
    elseif (Test-Path -LiteralPath $resolvedOutputRoot -PathType Container) {
        $latestDirectory = Get-ChildItem -LiteralPath $resolvedOutputRoot -Directory | Sort-Object LastWriteTime -Descending | Select-Object -First 1
        if ($null -ne $latestDirectory) {
            $outputPath = $latestDirectory.FullName
        }
    }

    $script:lastGeneratedOutputPath = $outputPath
...
function Select-OutputFolder {
    ...
    try {
        ...
        return [pscustomobject]@{ cancelled = $false; path = [IO.Path]::GetFullPath($dialog.SelectedPath) }
    }
    finally {
        $dialog.Dispose()
    }
...
try {
    try {
        ...
        try {
            ...
            $existingIsSameTool = ($health.signature -eq $toolSignature -and $health.basePath -eq $basePath)
        }
        catch {
            $existingIsSameTool = $false
        }
        ...
        try {
            ...
            try {
                ...
                if ($request.Method -eq 'POST') {
                    ...
                    if ($request.Path -in @('/api/save', '/api/generate', '/api/select-output-folder', '/api/open-output')) {
                ...
                switch ($route) {
                    'GET /api/health' {
                        Send-JsonResponse -Stream $stream -StatusCode 200 -Data @{ ok = $true; name = '注文書作成ツール'; signature = $toolSignature; basePath = $basePath }
                    }
                    'GET /api/data' {
                        $data = Read-OrderData
                    ...
                    'POST /api/select-output-folder' {
                        $payload = $request.Body | ConvertFrom-Json
                        $selection = Select-OutputFolder -CurrentPath (Get-PropertyText -Object $payload -Name 'currentPath')
                        Send-JsonResponse -Stream $stream -StatusCode 200 -Data @{ ok = $true; cancelled = $selection.cancelled; path = $selection.path }
                    }
                    'POST /api/open-output' {
                        ...
                        $fullGeneratedPath = if ([string]::IsNullOrWhiteSpace($script:lastGeneratedOutputPath)) { '' } else { [IO.Path]::GetFullPath($script:lastGeneratedOutputPath) }
                        if ([string]::IsNullOrWhiteSpace($fullGeneratedPath) -or
                            -not $fullRequestedPath.Equals($fullGeneratedPath, [StringComparison]::OrdinalIgnoreCase) -or
